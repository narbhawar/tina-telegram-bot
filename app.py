...
def check_scheduled_drops():
    ...
    for drop in pending:
        res = send_telegram(drop["user_id"], drop)
        drops.update_one({"_id": drop["_id"]}, {"$set": {"status": "sent"}})
        sent_log.insert_one({
...
scheduler.add_job(check_scheduled_drops, 'interval', minutes=1)
scheduler.start()

@app.route("/")
def home():
    return "Tina Bot + Scheduler is running!"
